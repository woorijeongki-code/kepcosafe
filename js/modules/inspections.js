            ...
            <div class="space-y-6 fade-in h-full flex flex-col relative pb-10">
                ...
                <div class="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl p-6 text-white shadow-md relative overflow-hidden mt-2 mx-2">
                    <div class="relative z-10 flex items-center justify-between">
                        <div>
                            <h2 class="text-xl font-black mb-1">?ºÏùº ?àÏ†Ñ?êÍ? Í≤∞Í≥º</h2>
                            <p class="text-cyan-50 text-xs font-medium">?ÑÏû• ?àÏ†Ñ?êÍ? ?òÌñâ Í≤∞Í≥ºÎ•??ïÏù∏?òÏÑ∏??</p>
                        </div>
                ...
                <div class="px-2 mb-2 flex justify-end">
                    <button id="btn-show-insp-form" class="bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-bold py-2 px-4 rounded-xl shadow-md transition-all flex items-center gap-1">
                        <i class="fa-solid fa-pen"></i> ?êÍ?Í≤∞Í≥º ?±Î°ù
                    </button>
                </div>
                ...
                <div id="insp-form-container" class="hidden bg-white rounded-2xl p-4 shadow-sm border border-slate-200 mx-2 mb-4 fade-in relative">
                    ...
                    <h4 class="font-bold text-slate-800 mb-4" id="insp-form-title">?àÏ†Ñ?êÍ? Í≤∞Í≥º ?±Î°ù</h4>
                    <form id="insp-form" class="space-y-3">
                        <input type="hidden" id="i-id" value="">
                        <div>
                            <label class="block text-xs font-medium text-slate-700 mb-1">?êÍ?Í≤∞Í≥º ?úÎ™©</label>
                            <input type="text" id="i-title" required class="block w-full p-2.5 border border-slate-300 rounded-xl focus:ring-cyan-500 focus:border-cyan-500 text-sm" placeholder="?? [Î∞∞Ï†Ñ] ?úÏÑ†Ï∞®Îüâ ?ºÏùº ?àÏ†Ñ?êÍ? Í≤∞Í≥º">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-700 mb-1">?êÍ? ?¥Ïö©</label>
                            <textarea id="i-content" rows="4" required class="block w-full p-2.5 border border-slate-300 rounded-xl focus:ring-cyan-500 focus:border-cyan-500 text-sm" placeholder="?ÑÏû• ?àÏ†Ñ?êÍ? Í≤∞Í≥º ?ÅÏÑ∏ ?¥Ïö©???ÖÎ†•?òÏÑ∏??></textarea>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-700 mb-1">Î∂ôÏûÑ (Ï≤®Î??åÏùº/?¨ÏßÑ)</label>
    ...
    init() {
        ...
        if (btnShowForm && formContainer) {
            btnShowForm.addEventListener('click', () => {
                ...
                document.getElementById('i-submit-btn').innerHTML = '<i class="fa-solid fa-check mr-1"></i> ?±Î°ù ?ÑÎ£å';
                document.getElementById('insp-form-title').textContent = '?àÏ†Ñ?êÍ? Í≤∞Í≥º ?±Î°ù';
                inspForm.reset();
            ...
            inspForm.addEventListener('submit', async (e) => {
                ...
                showLoading(false);
                if (error) {
                    alert('?Ä???§Ìå®: (Supabase ?åÏù¥Î∏îÏù¥ ?ÜÏùÑ ???àÏäµ?àÎã§) ' + error.message);
                } else {
                    alert('?±Í≥µ?ÅÏúºÎ°??Ä?•Îêò?àÏäµ?àÎã§.');
                    inspForm.reset();
                    formContainer.classList.add('hidden');
                    btnShowForm.classList.remove('hidden');
                    this.loadData();
                ...
                listEl.innerHTML = `
                    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex gap-4 cursor-pointer hover:bg-slate-50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="font-bold text-slate-800 mb-1">[Mock] ?•ÎßàÏ≤??ÑÏû• ?πÎ≥Ñ ?àÏ†Ñ?êÍ? Í≤∞Í≥º</h4>
                            <p class="text-xs text-slate-500 line-clamp-2">ÏßÄÎ∞?Ïπ®Ìïò ?ÑÌóò Íµ¨Ïó≠ Î∞??ëÏàòÍ∏??ïÏÉÅ ?ëÎèô ?¨Î? ?êÍ? ?ÑÎ£å (?πÏù¥?¨Ìï≠ ?ÜÏùå)</p>
                            <a href="#" target="_blank" rel="noopener noreferrer" class="mt-2 text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 py-1 px-2 rounded-lg flex items-center gap-1 w-fit transition-colors border border-slate-200" onclick="event.stopPropagation();">
                                <i class="fa-solid fa-paperclip"></i> ?ÑÏû•?¨ÏßÑ.jpg
                            </a>
        ...
        if (error || data.length === 0) {
            listEl.innerHTML = `
                <div class="text-center py-10 flex flex-col items-center">
                    ...
                    <p class="text-slate-500 text-sm font-medium">?±Î°ù???êÍ?Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§.</p>
                </div>
            `;
            return;
        }
                ...
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex gap-4 relative group hover:border-cyan-200 transition-colors">
                    ...
                    <div class="flex-1 min-w-0 pr-12">
                        ...
                        <a href="${item.file_url}" target="_blank" rel="noopener noreferrer" class="mt-2 text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 py-1 px-2 rounded-lg flex items-center gap-1 w-fit transition-colors border border-slate-200" onclick="event.stopPropagation();">
                            <i class="fa-solid fa-paperclip"></i> Ï≤®Î??êÎ£å ?ïÏù∏
                        </a>
                        ` : ''}
        ...
        if (isAdmin) {
            listEl.querySelectorAll('.btn-edit-insp').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    document.getElementById('insp-form-title').textContent = '?àÏ†Ñ?êÍ? Í≤∞Í≥º ?òÏ†ï';
                    document.getElementById('i-submit-btn').innerHTML = '<i class="fa-solid fa-check mr-1"></i> ?òÏ†ï ?ÑÎ£å';
            ...
            listEl.querySelectorAll('.btn-del-insp').forEach(btn => {
                btn.addEventListener('click', async (e) => {
                    e.stopPropagation();
                    if (confirm('???êÍ?Í≤∞Í≥ºÎ•??ïÎßê ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) {
                        if (!AppState.supabase) {
